...
# Define menus distintos por nível
if st.session_state["nivel"] == "admin":
    paginas = [
        "Cadastros Gerais", "Alunos", "Presenças",
        "Mensalidades", "Exames", "Equipamentos",
        "Empréstimos", "Admin System"
    ]
else:  # nivel == "user"
    paginas = [
        "Minhas Presenças", "Minhas Mensalidades",
        "Meus Exames", "Meus Empréstimos"
    ]

col1, col2 = st.columns([8,1])
with col1:
    pagina = st.radio("Menu", paginas, horizontal=True, label_visibility="collapsed")
with col2:
...
def pagina_alunos():
    ...
    if alunos:
        ...
        # Escolher aluno para editar
        nomes_alunos = [a["nome"] for a in alunos]
        aluno_sel = st.selectbox("Selecione o aluno para editar", nomes_alunos)
        if aluno_sel:
            aluno = col_alunos.find_one({"nome": aluno_sel})
            with st.form("form_editar_aluno"):
                nome = st.text_input("Nome", value=aluno["nome"])
                rg = st.text_input("RG", value=aluno.get("rg",""))
                faixa = st.selectbox("Faixa", ["Branca", "Amarela", "Laranja", "Verde", "Azul", "Roxa", "Marrom", "Preta"], index=["Branca", "Amarela", "Laranja", "Verde", "Azul", "Roxa", "Marrom", "Preta"].index(aluno.get("faixa","Branca")))
                data_nascimento = st.date_input("Data de Nascimento", value=datetime.strptime(aluno.get("data_nascimento","2000-01-01"), "%Y-%m-%d"))
                telefone = st.text_input("Telefone/WhatsApp (com DDD)", value=aluno.get("telefone",""), max_chars=15)
                if st.form_submit_button("Salvar Alterações"):
                    col_alunos.update_one({"_id": aluno["_id"]}, {"$set": {
                        "nome": nome,
                        "rg": rg,
                        "faixa": faixa,
...
if col_usuarios.count_documents({}) == 0:
    ...
    with col1:
        pagina = st.radio(
            "Menu",
            ["Cadastros Gerais", "Alunos", "Presenças", "Mensalidades", "Exames", "Equipamentos","Empréstimos"],
            horizontal=True,
    ...
    # Título com emojis japoneses
    st.markdown("# 🥋 空手道 (Karatedō) - Sistema de Karatê 🇯🇵")

    if pagina == "Cadastros Gerais":
        pagina_geral()
    elif pagina == "Alunos":
        pagina_alunos()
